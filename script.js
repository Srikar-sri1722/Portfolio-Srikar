...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Runs each feature in isolation - if one throws (e.g. a CDN script
    // like Chart.js failed to load on the deployed host), the rest of the
    // page still initializes instead of silently breaking everything
    // that comes after it.
    ...
    safeInit('particleBackground', initParticleBackground);
    safeInit('scrollProgress', initScrollProgress);
    safeInit('heroTyping', () => initHeroTyping(config));
    safeInit('education', () => renderEducation(config));
    safeInit('skills', () => renderSkills(config));
    safeInit('projects', () => renderProjects(config));
    safeInit('certifications', () => renderCertifications(config));
    safeInit('aiResearchSimulator', () => initAiResearchSimulator(config));
    safeInit('powerBiChart', initPowerBiChart);

    // Live Problem Solving Tracker
    safeInit('liveCodingEngine', () => initLiveCodingEngine(config));

    // AI & Command Palette Engine
    safeInit('aiChatbot', () => initAiChatbot(config));
    safeInit('commandPalette', () => initCommandPalette(config));
...
function initPowerBiChart() {
    ...
    if (typeof Chart === 'undefined') {
        console.warn('[Portfolio] Chart.js did not load - skipping Power BI chart.');
        return;
    }
    ...
    form.addEventListener('submit', async (e) => {
        ...
        if (form.action.includes('YOUR_FORM_ID')) {
            if (statusEl) {
                ...
                statusEl.textContent = 'Form is not connected to an email service yet — see setup instructions.';
            }
            return;
        }
        ...
        try {
            ...
            if (response.ok) {
                if (statusEl) {
                    ...
                    statusEl.textContent = "Thank you! Your message has been sent — Srikar will get back to you shortly.";
                }
                form.reset();
            } else {
            ...
            if (statusEl) {
                statusEl.style.color = '#f43f5e';
                statusEl.textContent = 'Something went wrong sending your message. Please email kandulasrikar172@gmail.com directly.';
            }
        } finally {
